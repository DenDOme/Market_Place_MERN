...
export const getRecommendedProducts = async (userId) => {
    try {
        const clickedCategories = await UserAction.distinct('categoryId', { userId });
        ...
        const recommendations = await Product.aggregate([
            ...
            {
                $addFields: {
                    clickScore: { $cond: [{ $in: ["$categoryId", clickedCategories] }, 1, 0] },
        ...
        console.error("Error getRecommendedProducts | userProductRecommend middleware", error);
